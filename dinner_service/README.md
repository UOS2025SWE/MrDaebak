...
- **비밀번호**: `testtest`
- **접속 방법**: 직원 로그인 페이지(`/login/staff`)에서 로그인
- **권한**: 직원 포지션 할당, 재고 관리, 회계 관리 등 전체 관리자 권한

### 직원 계정
#### 요리사 계정
- **이메일**: `cook@example.com`
- **비밀번호**: `testtest`
- **접속 방법**: 직원 로그인 페이지(`/login/staff`)에서 로그인
- **포지션**: COOK (요리사)
- **권한**: 조리 수락/완료
...
- **비밀번호**: `testtest`
- **접속 방법**: 고객 로그인 페이지(`/login/customer`)에서 로그인

> **참고**: 모든 테스트 계정의 비밀번호는 동일합니다(`testtest`). 실제 운영 환경에서는 각 계정별로 고유한 비밀번호를 설정해야 합니다.
...
신규 인벤토리/메뉴 로직을 빠르게 검증할 수 있도록 Pytest 기반의 단위 테스트가 추가되었습니다. 테스트는 루트 디렉터리에서 다음 명령으로 실행할 수 있습니다.

```bash
uv pip install -e .[dev]  # 최초 1회: 테스트 의존성 설치
pytest
```

주요 커버리지:
- `MenuService`의 스타일 가용성 계산이 재고 수량에 따라 비활성화되는지 검증
- `OrderService`의 사이드 디시 집계 로직(가격·재료 합산, 비활성화 메뉴 거부) 검증

CI 환경이 없다면 개발 시 수동으로라도 `pytest`를 실행해 재고/메뉴 회귀를 방지하세요.
