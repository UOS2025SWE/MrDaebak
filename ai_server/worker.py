...
try:
    ...
    from .services.llm import get_llm_service, unload_llm_service
    from .services.image import get_image_service, unload_image_service
except ImportError:
    try:
        from ai_server.services.stt import get_stt_service
        from ai_server.services.llm import get_llm_service, unload_llm_service
        from ai_server.services.image import get_image_service, unload_image_service
    except ImportError:
    ...
    try:
        ...
        if task_type == "transcribe":
            ...
            if not ENABLED_SERVICES["image"]:
                raise ValueError(
                    "Image Generation service is disabled on this worker.")

            # 1) 프롬프트를 먼저 LLM으로 번역 (한국어 -> 영어), 이때는 LLM이 켜져 있음
            raw_prompt = payload.get("prompt", "") or ""
            translated_prompt = raw_prompt.strip()

            if ENABLED_SERVICES["llm"] and translated_prompt:
                korean_pattern = re.compile(r"[가-힣]+")
                if korean_pattern.search(translated_prompt):
                    logger.info(
                        "Translating image prompt from Korean to English using LLM...")
                    llm = get_llm_service()
                    translation_prompt = (
                        "Translate the following Korean text to English. "
                        "Only output the English translation, nothing else.\n\n"
                        f"Korean text: {translated_prompt}\n\n"
                        "English translation:"
                    )
                    try:
                        english = await llm.generate(
                        ...
                        if english.startswith('"') and english.endswith('"'):
                            english = english[1:-1]
                        elif english.startswith("'") and english.endswith("'"):
                            english = english[1:-1]
                        translated_prompt = english or translated_prompt
                        logger.info("Prompt translated for image generation.")
                    except Exception as e:
                        logger.error(
                            f"Prompt translation failed, using original prompt: {e}")

            # 2) 이미지 생성을 위해 LLM 언로드(잠깐 내리기)
            if ENABLED_SERVICES["llm"]:
                logger.info(
                    "Temporarily unloading LLM to free GPU memory for image generation...")
                await unload_llm_service()
            ...
            # 4) 이미지 모델 언로드 후 LLM 다시 올리기
            logger.info("Unloading image model after generation...")
            await unload_image_service()
            if ENABLED_SERVICES["llm"]:
                logger.info("Reloading LLM model after image generation...")
                get_llm_service()
...
async def run_worker():
    ...
    parser.add_argument("--llm", action="store_true",
                        help="Enable LLM service")
    parser.add_argument("--image", action="store_true",
                        help="Enable Image Generation service")
    ...
    if args.all:
        ...
        if not any(ENABLED_SERVICES.values()):
            ...
            if len(sys.argv) == 1 and not any(k.startswith("ENABLE_") for k in os.environ):
                logger.info(
                    "No specific services enabled via args or env. Defaulting to ALL services.")
                ENABLED_SERVICES["stt"] = True
                ENABLED_SERVICES["llm"] = True
                ENABLED_SERVICES["image"] = True
    ...
    if ENABLED_SERVICES["image"]:
        # 이미지 모델은 GPU 메모리 사용량이 크므로, 실제 generate_image 요청 시점에 지연 로딩
        logger.info(
            "Image service enabled; model will be loaded lazily on first image task.")
